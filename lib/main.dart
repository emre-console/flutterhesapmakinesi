...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void _onPressed({String buttonText}) {
    ...
    if (Calculations.Operatorler.contains(buttonText)) {
      return setState(() {
    ...
    // On CLEAR press
    if (buttonText == Calculations.Temizle) {
      return setState(() {
        operations.add(Calculations.Temizle);
        calculatorString = "";
      });
    }

    // On Equals press
    if (buttonText == Calculations.ESIT) {
      ...
      return setState(() {
        ...
        operations.add(Calculations.ESIT);
        calculatorString = newCalculatorString;
        isNewEquation = false;
      });
    }

    if (buttonText == Calculations.NOKTA) {
      return setState(() {
    ...
    setState(() {
      if (!isNewEquation
          && operations.length > 0
          && operations.last == Calculations.ESIT
      ) {
        calculatorString = buttonText;
        isNewEquation = true;
      } else {
